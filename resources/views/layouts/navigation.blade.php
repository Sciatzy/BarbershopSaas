            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (Auth::user()->hasAnyRole(['Barbershop Admin', 'Branch Manager']))
                        ...
                        <x-nav-link :href="route('manager.barbers.index')" :active="request()->routeIs('manager.barbers.*')">
                            {{ __('Barbers') }}
                        </x-nav-link>
                    @endif
                    ...
                    @if (Auth::user()->hasRole('Barber'))
                        <x-nav-link :href="route('barber.dashboard')" :active="request()->routeIs('barber.dashboard')">
                            {{ __('Barber') }}
                        </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->hasAnyRole(['Barbershop Admin', 'Branch Manager']))
                ...
                <x-responsive-nav-link :href="route('manager.barbers.index')" :active="request()->routeIs('manager.barbers.*')">
                    {{ __('Barbers') }}
                </x-responsive-nav-link>
            @endif

            @if (Auth::user()->hasRole('Barbershop Admin'))
                <x-responsive-nav-link :href="route('billing.plans')" :active="request()->routeIs('billing.plans')">
                    {{ __('Billing') }}
                </x-responsive-nav-link>
            @endif

            @if (Auth::user()->hasRole('Barber'))
                <x-responsive-nav-link :href="route('barber.dashboard')" :active="request()->routeIs('barber.dashboard')">
                    {{ __('Barber') }}
                </x-responsive-nav-link>
            @endif
